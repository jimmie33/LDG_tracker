...
%         C = C/max(C);
        ...
        if config.verbose
            ...
            tracker.clsf = svmtrain_my( sample, label, 'boxconstraint',tracker.C,...
                'autoscale','false',statset('MaxIter',5000));
        end
    ...
    case 1 % liblinear: smoothing without storing svs
        ...
        tracker.clsf = trainll(label, sparse(sample),['-q -B 1',' -w1 ',c_pos,' -w0 ',c_neg]);
        tracker.clsf.w = w_old*0.95 + (tracker.clsf.w(1:end-1)/norm(tracker.clsf.w(1:end-1)))*0.05;
    ...
    case 2
        ...
        tracker.clsf = svmlibtrain(double(label),sample,['-q -b 1',' -w1 ',c_pos,' -w0 ',c_neg]);
