function [alphas offset] = seqminopt_my(data, targetLabels, boxConstraints, ...
...
%   $Revision: 1.1.6.7.14.2 $  $Date: 2011/03/17 22:25:59 $
...
if length(size(data)) ~= 2
    error('Bioinfo:svmtrain:InvalidInput', ...
        'TRAINING must be two-dimensional.');
end
...
if ~isequal(size(targetLabels), [nPoints 1])
    error('Bioinfo:svmtrain:InvalidInput', ...
        'Y size does not match to TRAINING');
end
...
if abs(linEqConstraint) > smoOptions.TolKKT
    error('Bioinfo:svmtrain:ViolatedPostcondition', ...
        'Linear equality is not satisfied.');
end
end % end of svmtrain
