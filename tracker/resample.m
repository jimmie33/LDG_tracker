...
temp = repmatls(svm_tracker.output,[size(sampler.patterns_dt,1),1]);
...
temp(1:end-1,1) = X(:)*config.pixel_step + sampler.roi(1)-1;
temp(1:end-1,2) = Y(:)*config.pixel_step + sampler.roi(2)-1;
...
% sampler.patterns_dt=zeros(sample_sz,numel(sampler.template));
